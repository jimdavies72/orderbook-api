...
exports.updateListData = async (req, res) => {
  try {
    // to do, see route
    res.status(503).send({ title: "Update list data", message: "Not currently implemented" });
...
exports.addListData = async (req, res) => {
  try {
    ...
    const currencyData = await response.json();
    const currencyList = Object.keys(currencyData.data);
    
    const listData = await Currency.create({unselected: currencyList});
    if (!listData) {
